## Hi there 👋

I'm Anton, a .NET Backend Developer focused on backend services, integrations and data processing.

### Languages and Tools

<p>
  <img src="https://skillicons.dev/icons?i=cs,dotnet,postgres,rabbitmq,docker,git,react,ts,grafana,github" />
  <img src="https://cdn.simpleicons.org/minio/C72E49" width="48" height="48" alt="MinIO" />
</p>

## Notable Projects

### [AccessFlow](https://github.com/h1mal1a1/AccessFlow)

Backend service for managing clients and user connections on VPS through the 3X-UI API.

![.NET](https://img.shields.io/badge/.NET-8-512BD4)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1)
![Docker](https://img.shields.io/badge/Docker-2496ED)
![EF Core](https://img.shields.io/badge/EF_Core-512BD4)

### [CosmeticEnterprise](https://github.com/h1mal1a1/CosmeticEnterprise)

Full-stack e-commerce application with .NET backend and React/TypeScript frontend.

![.NET](https://img.shields.io/badge/.NET-8-512BD4)
![React](https://img.shields.io/badge/React-TypeScript-61DAFB)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1)
![MinIO](https://img.shields.io/badge/MinIO-S3-C72E49)
![Docker](https://img.shields.io/badge/Docker-2496ED)

### [NotificationService](https://github.com/h1mal1a1/NotificationService)

Email notification microservice with RabbitMQ, retry policies and delivery logging.

![.NET](https://img.shields.io/badge/.NET-8-512BD4)
![RabbitMQ](https://img.shields.io/badge/RabbitMQ-FF6600)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1)
![Docker](https://img.shields.io/badge/Docker-2496ED)

### GitHub Metrics

<img src="./github-metrics.svg?v=2" alt="GitHub Metrics" />
